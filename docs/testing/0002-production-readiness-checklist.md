...
| 3 | Refunds module | ✅ Done (Wave C.1, design-0011) | Full lifecycle live-verified: request (server-computed proportional amounts, pending-reservation guard) → approve/reject → transactional complete (restock + `REFUND_IN` + sale `PARTIALLY_REFUNDED`→`REFUNDED`). Includes the sales-history dependency: `GET /sales(/:id)` + `/sales` pages. State machine = idempotency (documented deviation from api/0002 key scheme). |
...
| 6 | Customers / devices / payments / audit endpoints | 🟡 Customers done (Wave C.3, design-0012) | Customers CRUD + POS phone capture (upsert inside the sale tx, offline-safe) + wa.me direct-chat bill (₹0/message; Cloud API recorded as post-pilot opt-in). Devices / payments / audit-viewer endpoints remain. |
| 6b | Server-side PDF + public tokenized receipt link | ✅ Done (Wave C.2, design-0009 Gate 2 upgrade) | Stateless JWT share token minted by the authenticated invoice endpoint; `@Public()` `GET /public/receipts/:token(/pdf)` (30/min throttle, tamper → generic 404); `pdfkit` A6 receipt; customer page `/r/[token]`; WhatsApp text now carries the link. Live-verified: no-auth 200, tampered 404, real %PDF bytes. |
| 7 | Docs errata for route drift | ✅ Done (Wave C.2) | Errata table added to `api/0001` (purchases, sales sync, invoice receipt routes). |
...
| 9 | Web test runner + E2E test stabilization | ✅ Done (Wave D) | Jest via `next/jest`: 20 web unit tests (sync-worker contract, apiClient envelope + rotation-persistence lockstep, receipt-utils, formatters) wired into CI. Playwright smoke scaffold gated on `E2E_BASE_URL` (self-skips until a deployment exists) — supersedes the flaky real-DB Jest E2E path. Runner decision rationale retained below. |
